...
pub(super) async fn init_dns_config() -> Result<()> {
    ...
    if !dns_path.exists() {
        ...
        help::save_yaml(&dns_path, &default_dns_config, Some("# xpn DNS Config")).await?;
    }
...
async fn initialize_config_files() -> Result<()> {
        ...
        && !path.exists()
    {
        ...
        help::save_yaml(&path, &template, Some("# xpn"))
            .await
...
pub fn init_scheme() -> Result<()> {
    ...
    clash.set_value("", &"xpn")?;
    clash.set_value("URL Protocol", &"xpn URL Scheme Protocol")?;
...
#[cfg(target_os = "linux")]
const DEEP_LINK_SCHEMES: &[&str] = &["xpn"];
